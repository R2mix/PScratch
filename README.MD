...
| ![clones](web/assets/clones.png) | ` ArrayList<Sprite> SpriteClassArrayName = new ArrayList<Sprite>();` <br><br>  `SpriteClassArrayName.add(new SpriteClassName());` <br><br>  `stage.drawClones(SpriteClassArrayName);` <br><br>` deleteThisClone();`<br><br>` sortClones(ArrayList, String , boolean);`| The array that contains the class to clone <br><br><br><br> Add a new class instance in the ArrayList <br><br> Display each clone <br><br> Remove the clone from the ArrayList, should be call inside the class<br><br>can sort clones by a specific int or float parameter. For example: y coordinate. | 
