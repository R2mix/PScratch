...
| ![clones](web/assets/clones.png) | ` ArrayList<SpriteClass> SpriteClassArray = new ArrayList<SpriteClass>();` <br>  `SpriteClassArray.add(new SpriteClass());` <br>  `drawClones(SpriteClassArray)` <br>` SpriteClassArray.remove(this);`| The array that contains the class to clone <br><br> Add a new class instance in the ArrayList <br> Display each clone <br> Remove the clone from the ArrayList, should be call inside the class | 
