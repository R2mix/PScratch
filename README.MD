...
| ![clones](web/assets/clones.png) | ` ArrayList<SpriteClass> SpriteClassArray = new ArrayList<SpriteClass>();</br> SpriteClassArray.add(new SpriteClass()); ` | The array that contains the class to clone  |  | ` SpriteClassArray.add(new SpriteClass()); `| Add a new class instance in the ArrayList |
|  | ` for (int i = SpriteClassArray.size() - 1; i >= 0; i--) { }`| Iterate for each clones |
|  | ` SpriteClass SpriteClassInstance = SpriteClassArray.get(i); `| Get the current clone instance |
|  | ` SpriteClassInstance.draw(); `| Display each clone  |
