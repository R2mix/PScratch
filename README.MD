...
| ![clones](web/assets/clones.png) | ` ArrayList<SpriteClass> SpriteClassArray = new ArrayList<SpriteClass>();` <br> `SpriteClassArray.add(new SpriteClass());` <br> `for (int i = SpriteClassArray.size() - 1; i >= 0; i--) { }` <br> `SpriteClass SpriteClassInstance = SpriteClassArray.get(i);` <br> `SpriteClassInstance.draw();` | The array that contains the class to clone <br> Add a new class instance in the ArrayList <br> Iterate for each clones <br> Get the current clone instance <br> Display each clone | 
