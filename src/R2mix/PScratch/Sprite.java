...
public abstract class Sprite implements Runnable {
  ...
  public String getNAME(){
    ...
    // ??????? version with pg for insertion into a PGraphic, for now extended for
    // beta, future poly or over
    ...
    // call this function instead changing the boolean, more accurate to scratch but
    // both works
    ...
    private void rotationStyle() { // fixe rotation to none
        if (rotationStyle.equals("left-right")) {
        ...
        if (rotationStyle.equals("don't rotate")) {
            sens = 1;
        }
        if (rotationStyle.equals("all around")) {
    ...
    public boolean touch(float xx, float yy, float l, float h, float di, String rotationStyleOther) { // square hitbox
        ...
        if (rotationStyleOther.equals("all around")) {
    ...
    public boolean touch(Sprite other) {
        ...
        return touch(other.x, other.y, hL2, hH2, other.direction, other.rotationStyle) && other.display && display;
    }
    ...
    public boolean touch(String s) { // square hitbox comparing to :
        ...
        if (display) {
            // touch edge
            if (s.equals("edge") && (x + hL > myParent.width ||
                    x - hL < 0 ||
            ...
            // touch mouse
            else return s.equals("mouse") && touch(myParent.mouseX, myParent.mouseY, 0, 0, 0, "don't rotate");
        } else {
            return false;
        }
    }
    ...
    private void dragSprite() { // don't call it is automatique if you enable with the setDrageMode void
        if (draggable && myParent.mousePressed && touch("mouse")) {
            x = myParent.mouseX;
            y = myParent.mouseY;
        }
    }
