...
public class Sprite extends Thread {
  ...
  public void spriteFolder(String folderName) {
    costumes = tableauxImages.get(folderName);
  }
  ...
  public void moveRight(float a) {
    x += cos(angleRadian+ PI/2) * a;                                          // trigonometric calcul for go to the angle of the direction of the sprite
    y += sin(angleRadian+ PI/2) * a;
  }
  ...
  public void ifOnEdgeBounce() {                                           // bounce on edge of the screen
    if ( x+ spriteWidth/2 > width) {
      pointInDirection(180 - direction);
      setXTo(width - spriteWidth/2);
    }
    if (x - spriteWidth/2 < 0) {
      pointInDirection(180 - direction);
      setXTo(spriteWidth/2);
    }
    if (y + spriteHeight/2 > height) {
      pointInDirection(direction * -1);
      setYTo(height-spriteHeight/2);
    }
  ...
  public void changeColorEffectBy(int col) {
    ...
    colorEffectValue = int(map(colorCurrentValue%255, 0, 255, -1, -16777216/100));
  }
  public void setColorEffectTo(int col) {
    colorEffectValue = int(map(col%255, 0, 255, -1, -16777216/100));          // set color in binary to 0 to 100
  }
  ...
  //------------------------------------------------------------- detect colors behind the sprite ------------------------------------------------------------------
  ...
  public boolean touch(int col) {                                            // call this boolean for testing if touching color
    ...
    for (int i = 0; i < l; i++) {
      for (int j = 0; j < h; j++) {                                          // for only if the sprite has a color at this point
        if (get (int (x + i - l/2), int ( y + j - h/2)) == col && display  ) {
          colorTouched = true;
        }
      }
    }
  ...
  public boolean touch(float xx, float yy, float l, float h, float di) {     // square hitbox comparing two position and size
    ...
    float cx1 = x + spriteWidth/2 * cos(radians(direction)) - spriteHeight/2 * sin(radians(direction));
    float cy1 = y + spriteWidth/2 * sin(radians(direction)) + spriteHeight/2 * cos(radians(direction));
    float cx2 = x - spriteWidth/2 * cos(radians(direction)) - spriteHeight/2 * sin(radians(direction));
    float cy2 = y - spriteWidth/2 * sin(radians(direction)) + spriteHeight/2 * cos(radians(direction));
    float cx3 = x - spriteWidth/2 * cos(radians(direction)) + spriteHeight/2 * sin(radians(direction));
    float cy3 = y - spriteWidth/2 * sin(radians(direction)) - spriteHeight/2 * cos(radians(direction));
    float cx4 = x + spriteWidth/2 * cos(radians(direction)) + spriteHeight/2 * sin(radians(direction));
    float cy4 = y + spriteWidth/2 * sin(radians(direction)) - spriteHeight/2 * cos(radians(direction));
    float[] spriteXx = {cx1, cx2, cx3, cx4};
    float[] spriteYy = {cy1, cy2, cy3, cy4};

    float cxother1 = xx + l/2 * cos(radians(di)) - h/2 * sin(radians(di));
    float cyother1 = yy + l/2 * sin(radians(di)) + h/2 * cos(radians(di));
    float cxother2 = xx - l/2 * cos(radians(di)) - h/2 * sin(radians(di));
    float cyother2 = yy - l/2 * sin(radians(di)) + h/2 * cos(radians(di));
    float cxother3 = xx - l/2 * cos(radians(di)) + h/2 * sin(radians(di));
    float cyother3 = yy - l/2 * sin(radians(di)) - h/2 * cos(radians(di));
    float cxother4 = xx + l/2 * cos(radians(di)) + h/2 * sin(radians(di));
    float cyother4 = yy + l/2 * sin(radians(di)) - h/2 * cos(radians(di));
    ...
    if (showHitbox) {
      ...
      push();
      strokeWeight(10);
      point(cxother1, cyother1);
      point(cxother2, cyother2);
      point(cxother3, cyother3);
      point(cxother4, cyother4);
      pop();
    }
    for (int i = 0; i < 4; i++) {
      float mx = spriteXx[i] - xx;
      float my = spriteYy[i] - yy;
      float mxRot = mx * cos(-radians(di)) - my * sin(-radians(di));
      float myRot = mx * sin(-radians(di)) + my * cos(-radians(di));
      float mx2 = otherXx[i] - x;
      float my2 = otherYy[i] - y;
      float mxRot2 = mx2 * cos(-radians(direction)) - my2 * sin(-radians(direction));
      float myRot2 = mx2 * sin(-radians(direction)) + my2 * cos(-radians(direction));

      if ((mxRot  >= -l/2 && mxRot <= l/2 && myRot >= -h/2 && myRot <= h/2) || (mxRot2  >= -spriteWidth/2 && mxRot2 <= spriteWidth/2 && myRot2 >= -spriteHeight/2 && myRot2 <= spriteHeight/2) ) {
        returnTouch = true;
      }
    }
...
// -------------------------------------------------------- directoryFiles  ------------------------------------------------------------------
HashMap<String, PImage[]> tableauxImages;
...
public PImage[] spriteFolder(String folder) {
...
public class Stage {                                                        // similare to sprite class but more light
  ...
  public Stage( String folder) {
    String dossierPrincipal = sketchPath()+ "/data/";
    File dossier = new File(dossierPrincipal);
    File[] sousDossiers = dossier.listFiles();
    tableauxImages = new HashMap<String, PImage[]>();
    for (int i = 0; i < sousDossiers.length; i++) {
      ...
      if (sousDossier.isDirectory()) {
        ...
        tableauxImages.put(nomSousDossier, tableauImages);
      }
    }
    stage = tableauxImages.get(folder);
    for(int i = 0; i < stage.length; i++){
     stage[i].resize(width, height); 
    }
  }
  ...
  public void backdrops() {                                                 // call it in top of draw for being behing everything
    ...
    image(stage[backdrop], 0, 0, width, height );
    pop();
    textToScreen();
  }
