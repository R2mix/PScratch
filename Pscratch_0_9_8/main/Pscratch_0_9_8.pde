...
public class Sprite implements Runnable {
  ...
  public void spriteFolder(String folderName) {
    costumes = allAssetImages.get(folderName);                                 // create a reference to the mainImages
  }
  ...
  public boolean touch(float xx, float yy, float l, float h, float di) {     // square hitbox comparing two position and size
    ...
    float cy2 = y - hL * sin(radians(direction)) + hH * cos(radians(direction));
    float cx3 = x - hL * cos(radians(direction)) + hH * sin(radians(direction));
    float cy3 = y - hL * sin(radians(direction)) - hH * cos(radians(direction));
    float cx4 = x + hL * cos(radians(direction)) + hH * sin(radians(direction));
    float cy4 = y + hL * sin(radians(direction)) - hH * cos(radians(direction));
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if ((mxRot  >= -l/2 && mxRot <= l/2 && myRot >= -h/2 && myRot <= h/2) || (mxRot2  >= -hL && mxRot2 <= hL && myRot2 >= -hH && myRot2 <= hH) ) {
        returnTouch = true;
      }
    }
  ...
  public boolean touch(String s) {                                          // square hitbox comparing to :
    float hL =  spriteWidth/2 + hitboxL;
    float hH =  spriteHeight/2 + hitboxH;
    if (display) {
      // touch edge
      if (s == "edge" && ( x + hL > width ||
        x - hL < 0 ||
        y + hH > height ||
        y - hH < 0 )) {
        return true;
...
// -------------------------------------------------------- directoryFiles  ------------------------------------------------------------------
HashMap<String, PImage[]> allAssetImages;                                   // storing all arrays inside a dictionary
